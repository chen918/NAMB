...
while(k<=itl){
  ...
  p= pnorm(solve(diag(x=1,nrow=node)-rho*W)%*%(X%*%BBeta))
  ...
  while(i<=num){
    ...
    Rho[i] = Rho[i-1]+rnorm(1, mean = 0, sd = 0.2)
    ...
    proportion = (((1/max(Lamda)-Rho[i-1])*(Rho[i-1]-1/min(Lamda)))/((1/max(Lamda)-Rho[i])*(Rho[i]-1/min(Lamda))))*prod(unlist(lapply(seq(1,node),cal)))*
    ...
    betamt[,i] = betamt[,i-1]+rmvnorm(1, mean = rep(0,4), sigma=1.5*V)
