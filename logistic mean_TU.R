...
while(k<=itl){
  ...
  while(i<=num){
    ...
    proportion = (((1/max(Lamda)-Rho[i-1])*(Rho[i-1]-1/min(Lamda)))/((1/max(Lamda)-Rho[i])*(Rho[i]-1/min(Lamda))))*exp( sum(unlist(lapply(seq(1,node),cal)))+(t(Y)%*%solve(diag(1,node)-Rho[i]*W)%*%(X%*%betamt[,i-1]))-(t(Y)%*%solve(diag(1,node)-Rho[i-1]*W)%*%(X%*%betamt[,i-1])))*
